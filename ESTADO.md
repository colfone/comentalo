...
## Version actual: v4.39 — 24 de abril de 2026
...
| v4.39 | Sesión 24 abril (limpieza dead code abierta) | 24 abril 2026 | Migración `20260424130000_limpiar_abierta_en_rpcs.sql` aplicada en Supabase. Re-emite con `CREATE OR REPLACE` 5 funciones que mencionaban `'abierta'` en cláusulas `IN (...)` de SELECT/UPDATE como dead code post-consolidación v4.38. Cero cambios de lógica de negocio — el CHECK contraído en v4.38 ya garantizaba que no había filas con ese valor, así que `IN ('abierta', 'activa')` y `= 'activa'` devolvían el mismo row set. Funciones tocadas: (1) `asignar_intercambio` — CTE `locked_disponibles`, filtro reducido de `IN ('abierta', 'activa')` a `= 'activa'`; GRANT EXECUTE re-emitido; (2) `descontar_costo_crear_campana` (trigger `trg_costo_crear_campana`) — UPDATE de pausa cuando saldo llega a 0, `IN ('activa', 'abierta')` → `= 'activa'`; (3) `aplicar_creditos_intercambio` — UPDATE de pausa al creador cuando saldo llega a 0, mismo cambio; (4) `cerrar_campanas_vencidas` — UPDATE del cron diario, `IN ('abierta', 'activa', 'pausada')` → `IN ('activa', 'pausada')`; (5) `pausar_por_inactividad_calificacion` — UPDATE de pausa por inactividad, mismo cambio que (2)/(3). Triggers y pg_cron jobs no se re-crearon: `CREATE OR REPLACE FUNCTION` actualiza el cuerpo sin invalidar bindings por nombre. Cero matches restantes para `'abierta'` en código vivo de Supabase (las apariciones residuales están solo en migraciones históricas superseded, que son la fuente de verdad de qué se aplicó cuando — no se reescriben). |
| v4.38 | Sesión 24 abril (consolidar abierta → activa) | 24 abril 2026 | Consolidación del estado legacy `'abierta'` a `'activa'` en la tabla `campanas`. La dualidad venía del modelo v1 (PROYECTO.md ≤ v4.1) donde `abierta` era el primer estado vivo antes de llegar a 10 intercambios, y había quedado marcada como deuda en la migración `20260419200000_expand_campanas_estado.sql`. Resuelta en 2 pasos. **Paso 1 — código TS** (commit 6ddbf7a, 17 archivos, +33/−36 líneas): INSERTs en `/api/campanas/lanzar` y `/api/videos/registrar` ahora crean campañas con `estado='activa'`; filtros `.in("estado", ["abierta","activa"])` contraídos a `.eq("estado", "activa")`; arrays `ESTADOS_QUE_PERMITEN_FINALIZAR` y checks `.includes([...])` limpiados; checks `!== "abierta" && !== "activa"` simplificados a `!== "activa"`; union types `EstadoCampana` / `EstadoCampanaActiva` reducidos a 3 valores (`activa \| pausada \| finalizada`); `Record<EstadoCampana, ...>` para `ESTADO_LABEL` y `ESTADO_CLASSES` de `/admin/campanas` sin entradas duplicadas. **Paso 2 — migración** `20260424120000_consolidar_estado_abierta.sql` (aplicada en Supabase): (1) UPDATE defensivo a 'activa' (0 filas afectadas, verificado); (2) DROP + ADD `campanas_estado_check` contraído a `CHECK (estado IN ('activa', 'pausada', 'finalizada'))`; (3) `ALTER COLUMN estado SET DEFAULT 'activa'` (antes default era 'abierta' desde la migración inicial 20260416020535 — INSERT sin especificar estado hubiera reventado el CHECK nuevo). **Fix bug latente incluido:** `src/app/dashboard/intercambiar/page.tsx:272` filtraba solo `.eq("estado", "abierta")`, por lo que campañas que pasaron Pausar → Activar (y quedaron en `'activa'`) no contaban en el stat "Campañas activas" del card del user. **Dead code que queda como deuda nueva:** varios RPCs activos (`asignar_intercambio`, `cerrar_campanas_vencidas`, `pausar_por_inactividad_calificacion`, `aplicar_creditos_intercambio`, triggers de `loggear_movimientos_creditos`) mencionan `'abierta'` en cláusulas `IN (...)` de SELECT — no rompen nada porque el CHECK garantiza que no puede haber filas con ese valor, pero son referencias muertas. Se limpiarán en las próximas migraciones donde esos RPCs se re-emitan por cualquier otra razón. |
| v4.37 | Sesión 24 abril (enforcement estados usuario) | 24 abril 2026 | Enforcement de estados de usuario en 2 partes. **Parte A — gate en `src/app/dashboard/layout.tsx`** (commit 9c0c2a1): extiende el `select` con `estado, estado_motivo, estado_hasta`; si `estado !== 'activo'` retorna pantalla de bloqueo antes de renderizar header y children. `eliminado` (soft-delete) → client island `estado-eliminado-signout.tsx` que hace signOut silencioso + redirect a /login. `suspendido` / `baneado` → `estado-bloqueo.tsx` (client) con título según estado (baneado permanente vs suspendido con fecha en es-AR largo vs suspendido indefinido), subtítulo contextual, motivo opcional en card gris, CTA `mailto:soporte@comentalo.com` + botón "Cerrar sesión". Fail-open si la query a `usuarios` falla (mismo criterio que `canal_verificado`). Narrowing por igualdad estricta evita casts. **Parte B — RPC `asignar_intercambio`** (migración `20260424110000_asignar_intercambio_usuario_activo.sql`, aplicada en Supabase): (1) CTE `locked_disponibles` con nuevo `EXISTS (SELECT 1 FROM usuarios WHERE id = p_comentarista_id AND estado = 'activo')` como segundo filtro — guard defensivo para APIs llamadas directamente saltándose el layout; (2) CASE final con nuevo branch `USUARIO_NO_ACTIVO` al inicio (`'Tu cuenta no está activa.'`) antes que `USUARIO_SIN_VIDEO_ACTIVO` — orden importa para que un suspendido sin videos reciba el mensaje correcto. **Lo que NO cubre esta iteración:** endpoints de escritura fuera del RPC (verificar, calificar, crear campaña, registrar video, cancelar) — un usuario suspendido con sesión vieva puede llamar esos endpoints vía API directa. Queda pendiente como helper `requireActiveUserForApi()`. Las reservas ya tomadas antes de suspensión no se cancelan activamente — expiran solas en 2 min. |
| v4.36 | Sesión 24 abril (auto-expiración suspensiones) | 24 abril 2026 | Migración `20260424100000_auto_expiracion_suspensiones.sql` aplicada en Supabase (jobid=5). RPC `expire_suspensiones_temporales()` (plpgsql, SECURITY DEFINER, search_path=public): reactiva usuarios con `estado='suspendido' AND estado_hasta IS NOT NULL AND estado_hasta < now()` setéandolos a `activo` y limpiando `estado_motivo`/`estado_hasta`. Solo toca suspensiones temporales — las indefinidas (`estado_hasta IS NULL`) siguen requiriendo reactivación manual desde /admin/usuarios. pg_cron `expire-suspensiones-temporales` con cron `0 * * * *` (misma cadencia que pausar-por-inactividad-calificacion). Return JSON `{ok, expiradas}`. Cierra el primer pendiente de la sección "Enforcement de estados de usuario". |
| v4.35 | Sesión 24 abril (limpieza) | 24 abril 2026 | Eliminada la ruta huérfana `/dashboard/calificar/[campanaId]` — carpeta `src/app/dashboard/calificar/` borrada completa. La ruta quedó sin links entrantes desde v4.33 (se eliminó el botón "Calificar intercambios" del dashboard-client). Auditoría previa confirmó cero referencias `href`/`router.push`/`Link` en `src/`; las menciones de "calificar" que quedan apuntan al endpoint `/api/intercambios/calificar` (vivo, usado por `/dashboard/actividad` y `/dashboard/campana/[id]`) o a labels de `origen` en movimientos de créditos — sin relación con la página eliminada. |
| v4.34 | Sesión 24 abril | 24 abril 2026 | Pulido post-v4.33 — acciones admin + auth flows. (1) Menú "Acciones ▾" en /admin/campanas: 3 endpoints admin paralelos (/api/admin/campanas/pausar, activar, finalizar) con requireAdminForApi() + service_client sin check de ownership. Los endpoints de usuario rechazan 403 si el auth user no es dueño, por eso se crean paralelos. Client component campana-acciones-admin.tsx con dropdown position:fixed (mismo patrón que usuario-acciones-admin.tsx), items condicionales por estado: abierta/activa muestra Pausar, pausada muestra Activar, Finalizar siempre presente con modal rojo de confirmación (terminal e irreversible); finalizada renderiza `—` en vez del menú. Fix overflow-x-auto en el contenedor de la tabla. Commits 172111f + 092697a. (2) Botón "Cerrar sesión" en /verificar-canal — evita el loop donde un usuario con sesión pero sin fila en usuarios queda atrapado (la guard de /dashboard lo redirigía de vuelta a /verificar-canal). Mismo patrón del botón de /dashboard/perfil: signOut() + router.replace("/login"). Commit 70564b3. (3) Bypass admin en landing / — nuevo archivo src/lib/admin-emails.ts con ADMIN_EMAILS + isAdminEmail, puro sin imports server-only. admin-guard.ts trae next/headers via ./server al tope y no puede importarse desde client components; ahora admin-guard importa desde el nuevo archivo y lo re-exporta para mantener compat con imports existentes server-side (/auth/callback, etc). page.tsx (client) importa directamente de @/lib/admin-emails y redirige a /admin si isAdminEmail(user.email), /dashboard si no. Build de / pasó de 500 (server-only import error) a 200. Commit 8b122a0. (4) "Salir del panel" en /admin hace signOut() — antes era <Link href="/dashboard"> y mantenía la sesión activa, solo cambiaba de contexto. Ahora es AdminSignOutButton client island (nueva carpeta src/app/admin/admin-signout-button.tsx) que hace signOut() + router.replace("/login"); el layout sigue siendo server. ExitIcon migrado al client component. Commit 24c1ddd. |
| v4.33 | Sesión 23 abril (cierre 5) | 23 abril 2026 | Limpieza completa de estados legacy completada/calificada. Venían del modelo v1 (cerraban al llegar a 10 intercambios) y quedaron muertos en v2 — nada los transicionaba. Migración 20260423200000_limpiar_estados_legacy_campanas.sql: UPDATE defensivo a 'finalizada' + CHECK contraído a ('abierta', 'activa', 'pausada', 'finalizada'). 7 archivos de código limpiados: (1) /api/intercambios/calificar — eliminado bloque inalcanzable `if (estado === "completada")` que transicionaba a calificada, más la query count sinCalificar que quedaba huérfana. (2) dashboard-client.tsx — videosCompletados filtra por "finalizada"; branches de UI de completada/calificada removidos del badge del card (pausada naranja "Pausada", finalizada gris "Finalizada", resto gris "En curso"); botón "Calificar intercambios" eliminado (apuntaba a /dashboard/calificar/[id], era inalcanzable). (3) /dashboard/perfil — stat campañasCompletadas ahora usa `.eq("estado", "finalizada")`. (4) /dashboard/campana/[id] — labels del hero cubren los 4 estados reales (CAMPAÑA ACTIVA / PAUSADA / FINALIZADA). (5) /admin/campanas — union type EstadoCampana contraído a 4; ESTADO_LABEL/ESTADO_CLASSES simplificados; tab "finalizadas" filtra `["finalizada"]`. (6) /dashboard/actividad — comentario huérfano sobre auto-transición a "calificada" eliminado. (7) /dashboard/campanas — comentario stale sobre estados terminales actualizado. Balance: −58/+52 líneas. La dualidad abierta ↔ activa se deja intacta intencionalmente (deuda marcada en migración 20260419200000). Commit 1370537. |
| v4.32 | Sesión 23 abril (cierre 4) | 23 abril 2026 | Verificación end-to-end del trigger trg_creditos_bienvenida: al registrar usuario nuevo se loguea correctamente en movimientos_creditos con origen='bienvenida' (saldo_anterior=0, saldo_nuevo=creditos_bienvenida, monto=creditos_bienvenida, motivo=NULL, admin_id=NULL). Cierra el audit trail de los 5 orígenes del CHECK de movimientos_creditos. |
| v4.31 | Sesión 23 abril (cierre 3) | 23 abril 2026 | (1) Modal crear campaña con 2 variantes — nuevo useEffect fetchea saldo_creditos al montar via browser client (RLS select own). Modal con saldo suficiente muestra "Tu saldo actual: X 💎 / Se descontarán Y / Saldo después: Z". Modal con saldo insuficiente: título "Créditos insuficientes" + mensaje motivacional + 3 botones (Cancelar + Comprar créditos → + Ir a comentar →). Fix bug: cuando la respuesta trae creditos_insuficientes=true, la pantalla success ahora muestra cuadro ámbar con mensaje correcto ("Registramos tu video, pero no pudimos crear la campaña: te faltan créditos") en lugar del mensaje naranja engañoso de vistas insuficientes. Commit 3fc3a25. (2) /dashboard/comprar placeholder — página "Próximamente" sin lógica de pagos (respeta CLAUDE.md regla 1 / BACKLOG FASE 2). CTAs "Volver al inicio" y "Ganar créditos comentando →". Commit 3fc3a25. (3) Overlay "YA REGISTRADO" → "EN CAMPAÑA" coherente con estado vivo — mis-videos-youtube: markRegistered → markInCampaign con JOIN campanas!inner filtrando estado IN ('abierta', 'activa', 'pausada'). Payload rename ya_registrado → en_campana en los 7 usos del frontend. Endpoint /api/videos/registrar refactorizado: si el video existe con campaña viva → 409 "Este video ya tiene una campaña activa". Si existe sin campaña viva → reutiliza la fila existente (skip del INSERT a videos) y crea campaña nueva encima (permite relanzar tras finalizar/eliminar). Si no existe → INSERT como antes. El cache de videos YouTube (TTL 60min) queda con campo ghost ya_registrado hasta la próxima expiración; markInCampaign sobrescribe con en_campana fresco. Commit eea5a26. |
| v4.30 | Sesión 23 abril (cierre 2) | 23 abril 2026 | (1) Modal "Ver historial 💎" en /admin/usuarios — endpoint GET /api/admin/usuarios/historial-creditos, tabla con Fecha/Monto/Saldo resultante/Origen/Motivo, spinner de carga, estado vacío (commit dabeaf1). (2) Módulo /admin/campanas — página read-only con filtro por estado en tabs (Todas/Activas/Pausadas/Finalizadas), tabla con thumbnail+título, creador, badge de estado coloreado (verde abierta/activa, ámbar pausada, gris completada/calificada/finalizada), comentarios recibidos, fecha. Item "Campañas" agregado al sidebar del admin con MegaphoneIcon. Fix whitespace-nowrap en celda fecha. Commits 52f2bce + 89f425b. (3) Migración 20260423180000_loggear_movimientos_creditos.sql — logging de los 4 orígenes automáticos en movimientos_creditos: bienvenida (trigger otorgar_creditos_bienvenida), crear_campana (trigger descontar_costo_crear_campana), comentar (RPC aplicar_creditos_intercambio — 2 INSERTs: comentarista +1 y creador -1 con guard IF delta<>0 para evitar no-ops del piso GREATEST), calificar (RPC aplicar_credito_calificacion). Commit 13633f6. (4) Eliminación del límite de 2 videos activos simultáneos — removido del endpoint /api/videos/registrar (variable maxVideosActivos + check 409) y del dashboard-client.tsx (variables videosActivos/puedeRegistrar + branch else del aviso). Migración 20260423190000_remover_max_videos_activos.sql borra la fila de configuracion. En el modelo v2 el único limitador es el saldo de créditos. Commit 7970272. (5) Historial de créditos para el usuario — nueva página /dashboard/creditos con hero de saldo actual + tabla de movimientos (Fecha, Monto con signo y color verde/rojo, Saldo resultante, Origen con label legible, Motivo). Endpoint GET /api/usuarios/historial-creditos (service client porque movimientos_creditos tiene RLS cerrada). Dos entry points: click en badge 💎 del header convertido en Link y botón "Ver historial →" en card Créditos de /dashboard/perfil. Commit 5054e4e. |
| v4.29 | Sesión 23 abril (cierre) | 23 abril 2026 | Iteraciones sobre v4.28 + consistencia visual cross-pantallas. (1) /dashboard/campanas ahora filtra con `campanas!inner(...)` — INNER JOIN fuerza que solo videos con al menos una campaña aparezcan; videos sin campaña quedan fuera del listado. Limpieza del branch "Sin campaña activa" del subtítulo que ya no puede dispararse. (2) Vocabulario: contador "X/10 comentarios" → "X comentarios recibidos" (ya no hay techo de 10 por campaña en modelo v2). (3) Card Reputación del home /dashboard actualizada al nuevo formato consistente con /dashboard/perfil: getReputationLevel extendido con dot color (#dcdedf/#22c55e/#eab308/#f97316/#ef4444), badge con dot + label reemplaza el guión anterior, contador "X/20 calificados" cuando inactiva, promedio X.X★ cuando activa. (4) Pantalla de éxito "Campaña creada" en /dashboard/registrar-video muestra dos CTAs lado a lado en contenedor flex-wrap: "Ir a mis campañas" (secundario con borde morado) + "Ir a comentar" (primario gradiente). (5) Iteración doble en CampanaAcciones: primera versión mostraba los 4 botones siempre con opacity-40 + cursor-not-allowed para estados deshabilitados, segunda versión removió el botón "Lanzar campaña" y volvió al gate `return null` cuando no hay campaña (el filtro !inner hace que ese caso ya no ocurra en esta pantalla; se deja pendiente UX específica). Handler lanzarCampana removido del parent. (6) Limpieza manual de DB: video "doberman" (id e18c4653) sin campaña asociada eliminado manualmente desde Supabase para validar el filtro !inner. |
| v4.28 | Sesión 23 abril (nav + separación campañas/perfil) | 23 abril 2026 | Reestructuración de navegación del dashboard: nueva ruta /dashboard/campanas con la gestión de campañas (lista de videos con chip de estado, acciones Pausar/Activar/Finalizar/Eliminar, ConfirmModal, toast de reembolso, CTA "Crear campaña") — antes vivía dentro de /dashboard/perfil. Nav del layout pasa de 4 a 5 items: Inicio · Comentar · Mi actividad · Mis campañas · Perfil (nuevo MegaphoneIcon inline SVG). /dashboard/perfil reducido a info personal: hero con avatar + nombre + suscriptores + fecha registro + link "Ver canal" (agregada columna canal_url al SELECT), card Reputación prominente con dot + promedio X.X★ cuando ≥20 calificados o contador X/20 mientras "Sin activar", grid Stats con 4 tarjetas (comentarios realizados total + comentarios este mes + campañas completadas + calificación positiva %), card Créditos standalone grande con 💎 saldo_creditos, card Cuenta con email vinculado desde auth.user.email + botón Cerrar sesión. Fix limpieza: eliminado import de useRouter/useState innecesarios, eliminados iconos Home/Swap/Inbox/User/Bell del perfil (ya vivían en el layout). Limpieza CLAUDE.md: líneas meta embebidas removidas en commit 84a8dc6. |
...
| estado | TEXT | NOT NULL, DEFAULT 'abierta', CHECK IN ('abierta', 'activa', 'pausada', 'finalizada') |
...
| `20260423180000_loggear_movimientos_creditos.sql` | Logging de los 4 orígenes automáticos en movimientos_creditos (bienvenida, crear_campana, comentar ×2, calificar) | Aplicada |
| `20260423190000_remover_max_videos_activos.sql` | DELETE de la fila max_videos_activos en configuracion (obsoleta en modelo v2 — el único limitador es el saldo de créditos) | Aplicada |
...
- Enforcement en endpoints de escritura — el gate del layout y el filtro del RPC `asignar_intercambio` cubren el flujo principal (v4.37), pero los endpoints `/api/intercambios/verificar`, `/api/intercambios/calificar`, `/api/intercambios/cancelar`, `/api/campanas/lanzar`, `/api/videos/registrar` y `/api/canal/verificar-codigo` todavía aceptan requests de usuarios suspendidos/baneados con sesión vigente. Crear helper `requireActiveUserForApi()` (paralelo a `requireAdminForApi`) y aplicarlo a los 6 endpoints
- Email via Resend cuando créditos llegan a 0
- Módulo de compra de créditos /dashboard/comprar — placeholder creado en v4.31, implementación real en BACKLOG FASE 2 (ver PROYECTO.md sección 7)
- Ecosistema administrativo:
  - Panel admin completado: resumen, configuración, usuarios, ajuste manual de créditos con audit trail, campañas read-only (v4.30), historial de créditos por usuario (v4.30)
  - Pendiente: módulo de moderación — videos suspendidos, baneos
- UX videos sin campaña en /dashboard/campanas — hoy el filtro `campanas!inner` los oculta. Decidir si mostrarlos con un botón "Crear campaña" inline por video (requiere cambiar el INNER JOIN a LEFT JOIN y diferenciar el estado vacío) o mantenerlos fuera del listado como ahora
