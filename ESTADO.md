...
## Version actual: v4.38 — 24 de abril de 2026
...
- Limpieza de referencias muertas a `'abierta'` en RPCs vivos — tras la consolidación v4.38, los RPCs `asignar_intercambio`, `cerrar_campanas_vencidas`, `pausar_por_inactividad_calificacion`, `aplicar_creditos_intercambio` y triggers de `20260423180000_loggear_movimientos_creditos.sql` todavía mencionan `'abierta'` en cláusulas `IN (...)` de SELECT. Es dead code inofensivo (el CHECK lo previene) pero ruido semántico. Eliminar cuando esos RPCs se re-emitan por cualquier otra razón
